...
class DatadogScreenboardState(nixops.resources.ResourceState):
    ...
    def __init__(self, depl, name, id):
        ...
        self._screen_url = nixops_datadog.datadog_utils.get_base_url() + "screen/"
    ...
    def get_physical_spec(self):
        return (
            {"url": self._screen_url + self.screenboard_id}
            if self.screenboard_id
            else {}
        )
    ...
    def create_screenboard(self, defn, widgets, template_variables, read_only):
        response = self._dd_api.Screenboard.create(
            board_title=defn.config["boardTitle"],
            description=defn.config["description"],
            widgets=widgets,
            template_variables=template_variables,
            width=defn.config["width"],
            height=defn.config["height"],
            read_only=read_only,
        )
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        template_variables = nixops_datadog.datadog_utils.get_template_variables(
            defn=defn
        )
        ...
        if self.state != self.UP:
            self.log(
                "creating datadog screenboard '{0}...'".format(
                    defn.config["boardTitle"]
                )
            )
            screenboard_id = self.create_screenboard(
                defn=defn,
                widgets=widgets,
                template_variables=template_variables,
                read_only=read_only,
        ...
        if self.state == self.UP:
            if self.screenboard_exist(self.screenboard_id) == False:
                self.warn(
                    "datadog screenboard with id {0} doesn't exist anymore.. recreating ...".format(
                        self.screenboard_id
                    )
                )
                screenboard_id = self.create_screenboard(
        ...
        with self.depl._db:
            ...
            if screenboard_id != None:
                self.screenboard_id = screenboard_id
    ...
    def _destroy(self):
        if self.state == self.UP:
            ...
            if self.screenboard_exist(self.screenboard_id) == False:
                self.warn(
                    "datadog screenboard with id {0} already deleted".format(
                        self.screenboard_id
                    )
                )
            else:
                self.log(
                    "deleting datadog screenboard ‘{0}’...".format(self.board_title)
                )
                response = self._dd_api.Screenboard.delete(self.screenboard_id)
                if "errors" in response.keys():
                    raise Exception(
                        "there was errors while deleting the screenboard: {}".format(
                            str(response["errors"])
                        )
                    )
