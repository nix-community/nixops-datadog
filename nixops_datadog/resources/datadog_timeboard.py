...
class DatadogTimeboardState(nixops.resources.ResourceState):
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        if self.state != self.UP:
            ...
            timeboard_id, url = self.create_timeboard(
                defn=defn,
                graphs=graphs,
                template_variables=template_variables,
                read_only=read_only,
        ...
        if self.state == self.UP:
            if self.timeboard_exist(self.timeboard_id) == False:
                self.warn(
                    "datadog timeboard with id {0} doesn't exist anymore.. recreating ...".format(
                        self.timeboard_id
                    )
                )
                timeboard_id, url = self.create_timeboard(
        ...
        with self.depl._db:
            ...
            if timeboard_id != None:
                self.timeboard_id = timeboard_id
    ...
    def _destroy(self):
        if self.state == self.UP:
            ...
            if self.timeboard_exist(self.timeboard_id) == False:
                self.warn(
                    "datadog timeboard with id {0} already deleted".format(
                        self.timeboard_id
                    )
                )
            else:
                ...
                if response and "errors" in response:
                    raise Exception(
                        "there was errors while deleting the timeboard: {}".format(
                            str(response["errors"])
                        )
                    )
